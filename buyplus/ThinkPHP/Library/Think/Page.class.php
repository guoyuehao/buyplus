...
class Page{
    ...
    public $offset; // 最后一页是否显示总页数
    public $offsets; // 最后一页是否显示总页数
    ...
    private $config  = array(
        ...
        'first'  => '首页',
        'last'   => '尾页',
        'theme'  => '%FIRST% %UP_PAGE% %LINK_PAGE% %DOWN_PAGE% %END% %OFFSET% %OFFSTES%',
    );
    ...
    public function __construct($totalRows, $listRows=20, $parameter = array()) {
        ...
        $this->offset   = $this->listRows * ($this->nowPage-1)+1;
        $this->offsets   = $this->listRows * $this->nowPage;
    ...
    public function show() {
        ...
        if($this->offsets > $this->totalRows){
            $this->offsets = $this->totalRows;
        }

        //替换分页内容
        $page_str = str_replace(
            array('%HEADER%', '%NOW_PAGE%', '%UP_PAGE%', '%DOWN_PAGE%', '%FIRST%', '%LINK_PAGE%', '%END%', '%TOTAL_ROW%', '%TOTAL_PAGE%','%OFFSET%','%OFFSETS%'),
            array($this->config['header'], $this->nowPage, $up_page, $down_page, $the_first, $link_page, $the_end, $this->totalRows, $this->totalPages,  $this->offset, $this->offsets),
            $this->config['theme']);
        return "{$page_str}";
    }
}
